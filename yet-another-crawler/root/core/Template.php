...
class Template {
  ...
  protected function loadAdapter() {
    ...
    $adapterClass = ucfirst($config->paths->app->adapters->common()) . '_' . ucfirst($config->paths->app->adapters->templates()) . '_' . ucfirst($config->templates->adapter());
    $this->adapter = new $adapterClass;
    $this->adapter->setTemplateDir($config->paths->app->views());
  }
